...
/* ASSISTANT PATCH: v44 entry init */
(async function(){
  try{
    if(window.BTLIDER && typeof window.BTLIDER.loadTemplates === 'function'){
      await window.BTLIDER.loadTemplates();
    }
    if(window.BTLIDER && typeof window.BTLIDER.show === 'function'){
      // show guest first; if supabase session exists, you can switch to authed later
      window.BTLIDER.show('guest');
    }
  }catch(e){
    console.error("[BTLIDER] init failed", e);
  }
})();
